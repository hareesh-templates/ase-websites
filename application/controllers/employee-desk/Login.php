...
class Login extends CI_Controller {
	...
	public function index()
	{
        ...
        if($this->input->post('loginSubmit')){ 
           ...
           if($this->form_validation->run() == true){ 
                ...
                $checkLogin = $this->emp_user->getRows($con);
                if($checkLogin){ 
                    $this->session->set_userdata('isUserLoggedIn', TRUE); 
                    $this->session->set_userdata('id', $checkLogin['id']); 
                    redirect('employee-desk/home'); 
                }else{
		...
		$this->load->view('employee-desk/eswarihomeslogin');
	}
    ...
    public function register(){
        ...
        $this->load->view('employee-desk/register', $data);
    }
    ...
    public function email_check($str){ 
        ...
        $checkEmail = $this->emp_user->getRows($con); 
        if($checkEmail > 0){ 
